...
int  linstr( char *instr )
{
	...
	if (instr != NULL)
		while (*instr) {
			...
			/* Insertion error? */
			if (status != TRUE) {
				mlwrite(TEXT168);	/* "%%Can not insert string" */
				break;
			}
			instr++;
		}
...
char * getctext()	/* grab and return a string with the text of the current line */
{
...
int putctext( char *iline )	/* replace the current line with the passed in text	*/
{
	register int status;
...
int  clear_ring( int f, int n )
{
	register int index;

	for (index = 0; index < NRING; index++)
		next_kill();
	mlwrite(TEXT228);	/* "[Kill ring cleared]" */
	return(TRUE);
}
