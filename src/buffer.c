...
int usebuffer( int f, int n )
{
	...
	bp = getdefb();
	bp = getcbuf(TEXT24, bp ? bp->b_bname : mainbuf, TRUE);		/* "Use buffer" */
	if (!bp)
		return(ABORT);
...
int nextbuffer( int f, int n )	/* switch to the next buffer in the buffer list */
{
...
int killbuffer( int f, int n )
{
	...
	bp = getdefb();
	bp = getcbuf(TEXT26, bp ? bp->b_bname : mainbuf, TRUE);		/* "Kill buffer" */
	if (bp == NULL)
		return(ABORT);
...
int popbuffer( int f, int n )
{
	register BUFFER *bp;	/* ptr to buffer to dump */

	/* get the buffer name to pop */
	bp = getdefb();
	bp = getcbuf(TEXT27, bp ? bp->b_bname : mainbuf, TRUE);		/* "Pop buffer" */
	if (bp == NULL)
		return(ABORT);
...
int zotbuf( BUFFER *bp )	/* kill the buffer pointed to by bp */
{
	...
	if (bp->b_nwnd != 0) {
		mlwrite(TEXT28);		/* "Buffer is being displayed" */
		return(FALSE);
	}

	/* we can not kill an executing buffer */
	if (bp->b_exec != 0) {
		mlwrite(TEXT226);		/* "%%Can not delete an executing buffer" */
		return(FALSE);
	}
...
ask:	if (mlreply(TEXT29, bufn, NBUFN) != TRUE)	/* "Change buffer name to: " */
		return(FALSE);
...
int makelist( int iflag )
{
	...
	if (addline(blistp, TEXT30) == FALSE	/* "ACTN   Modes       Size Buffer	   File" */
	...
	*cp1++ = ' ';
	...
	for (i = 0; i < NUMMODES; i++)
		if (gmode & (1 << i))
			...
			*cp1++ = '.';
	strcpy(cp1, TEXT31);			/* "	      Global Modes" */
...
void  flong_asc( char buf[], int width, long num )
{
...
int bclear( BUFFER *bp )
{
	...
	&& (s=mlyesno(TEXT32)) != TRUE)		/* "Discard changes" */
		return(s);
...
int  unmark( int f, int n )	/* unmark the current buffers change flag */
{
	/* unmark the buffer */
	curbp->b_flag &= ~BFCHG;
