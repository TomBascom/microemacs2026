...
int risearch( int f, int n )
{
	...
	backchar(TRUE, 1);		/* Make sure the search doesn't match where we already are by backing up a character.	 */
...
int  fisearch( int f, int n )		/* Again, but for the forward direction */
{
	register int	 status;
...
start_over:    /* This is a good place to start a re-execution: */
	...
	col = (clexec)? 0: mlprompt(TEXT165, pat, isterm);	/* "ISearch: " */
	cpos = 0;			/* Start afresh 		  */
	status = TRUE;			/* Assume everything's cool   */

	for (;;) {			/* ISearch per character loop */

		c = ectoc(expc = get_char());	/* Check for special characters first.	*/
		...
		TTforg(gfcolor);			/* set up the proper colors for the command line */
		TTbacg(gbcolor);
...
int  checknext( int chr, int dir )
{
	...
	if (dir == FORWARD) {	/* If searching forward		*/
		if ((sts = !boundry(curline, curoff, FORWARD))) {
			if ((sts = eq(nextch(&curline, &curoff, FORWARD), chr))) {	/* If it's what we're looking for, set the point and say that we've moved. */
	...
	if (sts == FALSE)			 /* If the 'next' character didn't fit in the pattern, let's go search for it somewhere else. */
		sts = scanmore(dir);
...
int  get_char()
{
	...
	if (cmd_offset >= CMDBUFLEN-1) {/* If we're getting too big ...	*/		/* Complain loudly and bitterly	*/
		mlwrite (TEXT167);	/* "? command too long" */
		return(isterm);		/* And force a quit		*/
	}

	if ((c = getkey()) == isterm) return (isterm);		/* If $isterm != meta character, then create the extended character as getcmd() does. */
