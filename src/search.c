...
/* forwsearch -- Search forward.  Get a search string from the user, and search for the string.
 * If found, reset the "." to be just after the match string, and (perhaps) repaint the display.
 */
...
int forwsearch( int f, int n )
{
	...
	if (n < 0)				/* If n is negative, search backwards.  Otherwise proceed by asking for the search string. */
...
int forwhunt( int f, int n )
{
	...
	if (pat[0] == '\0') {		/* Make sure a pattern exists, or that we didn't switch into MAGIC mode after we entered the pattern. */
		mlwrite(TEXT80);	/* "No pattern set" */
		return FALSE;
	}
	...
	if (searchtype == SRBEGIN) {	/* Do one extra search to get us past our current match, if the search type has us at the start of a match, instead of after a match. */
		spoint = PTBEG;
		if (lastflag & CFSRCH)
			n = (n > 2)? (n + 1): 2;
	}
...
int  backsearch( int f, int n )
{
	...
	if (n < 0)				/* If n is negative, search forwards. Otherwise proceed by asking for the search string. */
...
int  backhunt( int f, int n )
{
	...
	if (tap[0] == '\0') {		/* Make sure a pattern exists, or that we didn't switch into MAGIC mode after we entered the pattern. */
		mlwrite(TEXT80);	/* "No pattern set" */
		return FALSE;
	}
	...
	if (searchtype == SREND) {	/* Do one extra search to get us past our current match, if the search type has us at the start of a match, instead of after a match. */
		spoint = PTEND;
		if (lastflag & CFSRCH)
			n = (n > 2)? (n + 1): 2;
	}
...
int mcscanner( int direct, int beg_or_end, int repeats )
{
	MC	*mcpatrn;	/* pointer into pattern */
	LINE	*curline;	/* current line during scan */
	int	curoff;		/* position within current line */

	beg_or_end ^= direct;	/* if we are searching forwards, use the pattern in mcpat[].  Otherwise, use the reversed pattern in tapcm[].	 */
	mcpatrn = (direct == FORWARD)? &mcpat[0]: &tapcm[0];
	curline = curwp->w_dotp;		/* Setup local scan pointers to global "."	 */
	curoff  = curwp->w_doto;

	while (!boundry(curline, curoff, direct)) {		/* Scan each character until we hit the head link record. */
		matchline = curline;				/* Save the current position in case we need to restore it on a match */
		matchoff = curoff;
		matchlen = 0;					/* initialize matchlen to zero in case we are doing a search for replacement.	 */
		if (amatch(mcpatrn, direct, &curline, &curoff)) {	/* A SUCCESSFULL MATCH!!!  Flag that we have moved, reset the global "." pointers.	 */
			curwp->w_flag |= WFMOVE;
			...
			if (direct == REVERSE) {		/* If we're heading in reverse, set the "match" pointers to the start of the string, for savematch().  */
				matchline = curline;
				matchoff = curoff;
			}
			if (savematch() == ABORT)
				return ABORT;
			if (--repeats <= 0)			/* Continue scanning if we haven't found the nth match. */
				return TRUE;
		}
...
int amatch(MC *mcptr, int direct, LINE **pcwline, int *pcwoff)
{
	...
	curline = *pcwline;	/* Set up local scan pointers to "." and set up our local character counting variable, which can correct matchlen on a failed partial match. */
	curoff = *pcwoff;
	cl_matchlen = 0;

	while (mcptr->mc_type != MCNIL) {				/* Loop through the meta-pattern, laughing all the way. */
		if ((cl_type = (mcptr->mc_type & ALLCLOS))) {		/* Is the current meta-character modified by a closure?	 */
			if (cl_type == ZEROONE)
			{
				cl_min = 0;
			...
			{
				cl_min = (cl_type == CLOSURE_1);	/* Minimum number of characters that may match is 0 or 1. */
				while (mceq(nextch(&curline, &curoff, direct), mcptr)) {	/* Match as many characters as possible against the current meta-character. */
					cl_matchlen++;
				}
			}
			mcptr++;			/* We are now at the character that made us fail.  Try to match the rest of the pattern. Shrink the closure by one for each failure. */
		...
		{
			if (!mceq(nextch(&curline, &curoff, direct), mcptr))	/* A character to compare against the meta-character equal function. If we still match, increment the length.	 */
				return FALSE;
			matchlen++;
		}
		mcptr++;
...
success:
	*pcwline = curline;	/* A SUCCESSFULL MATCH!!! Reset the "." pointers. */
	*pcwoff  = curoff;
	return (TRUE);
}
...
int scanner( int direct, int beg_or_end, int repeats )
{
	...
	beg_or_end ^= direct;		/* If we are going in reverse, then the 'end' is actually the beginning of the pattern.  Toggle it.	 */
	if (direct == FORWARD) {	/* If we are searching forwards, use the pattern in pat[], and the jump size in lastchfjump.  	*/
		patrn = (char *)&pat[0];
		jump = lastchfjump;
	}
	else				/* Otherwise, use the reversed pattern in tap[], and the jump size of lastchbjump. */
	{
		patrn = (char *)&tap[0];
		jump = lastchbjump;
	}
	curline = curwp->w_dotp;	/* Set up local pointers to global "." */
	curoff = curwp->w_doto;

	if (!fbound(patlenadd, &curline, &curoff, direct)) {		/* Scan each character until we hit the head link record. */
		do
		{
			scanline = matchline = curline;
			scanoff  = matchoff  = curoff;
			patptr = patrn;
			while ((c = (unsigned char)(*patptr++)) != '\0')		/* Scan through the pattern for a match. */
				if (!eq(c, nextch(&scanline, &scanoff, direct)))
					goto fail;

			curwp->w_flag |= WFMOVE;	/* A SUCCESSFULL MATCH!!! Flag that we have moved and reset the global "." pointers.	 */
			if (beg_or_end == PTEND)	/* at end of string */
			{
...
void  setjtable()
{
	...
	for (i = 0; i < patlenadd; i++) {	/* Now put in the characters contained in the pattern, duplicating the CASE. */
		if (isletter(pat[i]))
			deltaf[(unsigned char) chcase(pat[i])]
			 = patlenadd - i;
		deltaf[(unsigned char) pat[i]] = patlenadd - i;

		if (isletter(tap[i]))
			deltab[(unsigned char) chcase(tap[i])]
			 = patlenadd - i;
		deltab[(unsigned char) tap[i]] = patlenadd - i;
	}
	 ...
	 */
	lastchfjump = patlenadd + deltaf[(unsigned char) tap[0]];
	lastchbjump = patlenadd + deltab[(unsigned char) pat[0]];

	if (isletter(tap[0]))
		deltaf[(unsigned char) chcase(tap[0])] = 0;
	deltaf[(unsigned char) tap[0]] = 0;

	if (isletter(pat[0]))
		deltab[(unsigned char) chcase(pat[0])] = 0;
	deltab[(unsigned char) pat[0]] = 0;
}
...
int savematch()
{
	...
	grpmatch[0] = regtostr(patmatch, &tmpreg);		/* Save the groups.	 */
	...
	while ((pchr = *patptr) && status) {
		switch (pchr) {
			case MC_CCL:
				...
				does_closure = TRUE;
				break;
			case MC_BOL:	/* If the BOL character isn't the first in the pattern, we assume it's a literal instead. */
				if (mj != 0)
					goto litcase;
				mcptr->mc_type = BOL;
				magical = TRUE;
				break;
			case MC_EOL:	/* If the EOL character isn't the last in the pattern, we assume it's a literal instead. */
				if (*(patptr + 1) != '\0')
					goto litcase;
				mcptr->mc_type = EOL;
				magical = TRUE;
			...
			case MC_CLOSURE:
			case MC_CLOSURE_1:
			case MC_ZEROONE:	/* Does the closure symbol mean closure here? If so, back up to the previous element and indicate it is enclosed. */
				...
				does_closure = FALSE;
				break;
			case MC_ESC:	/* No break between here and LITCHAR if the next character is to be taken literally. */
				magical = TRUE;
				pchr = *++patptr;
				if (pchr == MC_GRPBEG) {	/* Start of a group.  Indicate it, and set magical. */
					...
					{
						mlwrite(TEXT221);	/*			"Too many groups" */
						status = FALSE;
					}
					break;
				}
				else if (pchr == MC_GRPEND) {	/* If we've no groups to close, assume a literal character.  Otherwise, indicate the end of a group. */
					if (stacklevel > 0) {
...
litcase:			mcptr->mc_type = LITCHAR;
	...
	if (stacklevel) {
		status = FALSE;
		mlwrite(TEXT222);	/*			"Group not ended"	*/
	}
...
void mcclear()
{
	...
	for (j = 0; j < MAXGROUPS; j++) {	/* Remember that grpmatch[0] == patmatch. */
		if (grpmatch[j] != NULL) {
			free(grpmatch[j]);
			grpmatch[j] = NULL;
		}
...
int mceq( unsigned char bc, MC *mt)
{
	...
	switch (mt->mc_type & MASKCLO) {
		...
		case NCCL:
			...
			break;
		default:
			mlwrite(TEXT95, mt->mc_type);	/* "%%mceq: what is %d?" */
			result = FALSE;
			break;
	}	/* End of switch.*/
...
int cclmake( char **ppatptr, MC *mcptr )
{
	...
	if ((pchr = *patptr) == MC_ECCL) {
		mlwrite(TEXT96);		/* "%%No characters in character class" */
		free(bmap);
		return FALSE;
	}
	...
	*ppatptr = patptr;
	...
	if (pchr == '\0') {
		mlwrite(TEXT97);		/* "%%Character class not ended" */
		free(bmap);
		return FALSE;
	}
	return TRUE;
...
int biteq( int bc, EBITMAP cclmap )		/* biteq -- is the character in the bitmap? */
{
...
void  setbit( int bc, EBITMAP cclmap)		/* setbit -- Set a bit (ON only) in the bitmap. */
{
	if ((unsigned)bc < HICHAR)
