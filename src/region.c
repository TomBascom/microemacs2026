...
int reglines()		/* how many lines in the current region	used by the trim/entab/detab-region commands */
{
...
int narrow( int f, int n )	/* Narrow-to-region (^X-<) makes all but the current region inthe current buffer invisable and unchangable */
{
	...
	if (bp->b_flag&BFNAROW) {
		mlwrite(TEXT71);	/* "%%This buffer is already narrowed" */
		return(FALSE);
	}
...
int widen( int f, int n )	/* widen-from-region (^X->) restores a narrowed region	*/
{
	...
	if ((bp->b_flag&BFNAROW) == 0) {
		mlwrite(TEXT74);	/* "%%This buffer is not narrowed" */
		return(FALSE);
	}
	...
	bp->b_flag &= (~BFNAROW);
	mlwrite(TEXT75);		/* "[Buffer is widened]" */
        return(TRUE);
}
...
int getregion( REGION *rp )
{
        ...
        if (curwp->w_markp[0] == (LINE *)NULL) {
                mlwrite(TEXT76);		/* "No mark set in this window" */
                return(FALSE);
        }
...
char * regtostr( char *buf, REGION *region )
{
	register LINE	*linep;
	register int	loffs;
	register long	rsize;
...
char * getreg( char *value )	/* return some of the contents of the current region */
{
	REGION region;
...
int indent_region( int f, int n )	/* indent a region n tab-stops */
{
...
int undent_region( int f, int n)	/* undent a region n tab-stops */
{
	register int inc;	/* increment to next line [sgn(n)] */
	int i, count;
