...
int setfillcol( int f, int n )	/* Set fill column to n. */
{
	fillcol = n;
	mlwrite(TEXT59, n);	/* "[Fill column is %d]" */
	return (TRUE);
}
...
int showcpos( int f, int n )
{
	...
	mlwrite(TEXT60,			/* "Line %D/%D Col %d/%d Char %D/%D (%d%%) char = 0x%x" */
		predlines + 1, numlines + 1, col+1, ecol,
...
int getccol( int bflg )	/* Return current column.  Stop at first non-blank given TRUE argument. */
{
...
int findcol( LINE *lp, int pos )	/*	findcol: Return display column in line at char position	*/
{
	register int c, i, col;
...
int setccol( int pos )	/* Set current column. */
{
...
int detab( int f, int n ) 	/* change tabs to spaces */
{
...
int entab( int f, int n ) 	/* change spaces to tabs where posible */
{
	register int inc;	/* increment to next line [sgn(n)] */
...
int trim( int f, int n )	/* trim:	trim trailing whitespace from the point to eol with no arguments, it trims the current region */
{
...
int insbrace( int n, int c )	/* insert a brace into the text here...we are in CMODE */
{
...
int setmod( int f, int n )	/* prompt and set an editor mode */
{
	return (adjustmode(TRUE, FALSE));
}

int delmode( int f, int n )	/* prompt and delete an editor mode */
{
	return (adjustmode(FALSE, FALSE));
}

int setgmode( int f, int n )	/* prompt and set a global editor mode */
{
	return (adjustmode(TRUE, TRUE));
}

int delgmode( int f, int n )	/* prompt and delete a global editor mode */
{
	return (adjustmode(FALSE, TRUE));
}

int adjustmode( int kind, int global )	/* change the editor mode status */
{
	...
	if (global)
		strcpy(prompt, TEXT62);	/* "Global mode to " */
	else
		strcpy(prompt, TEXT63);	/* "Mode to " */

	if (kind == TRUE)
		strcat(prompt, TEXT64);	/* "add: " */
	else
		strcat(prompt, TEXT65);	/* "delete: " */
	...
	mlwrite(TEXT66);	/* "No such mode!" */
	return (FALSE);
}

int clrmes( int f, int n )	/*	This function simply clears the message line, mainly for macro usage			*/
{
	mlforce("");
	return (TRUE);
}

int writemsg( int f, int n )		/*	This function writes a string on the message line mainly for macro usage			*/
{
	register int status;
	char buf[NPAT];		/* buffer to recieve message into */

	if ((status = mlreply(TEXT67, buf, NPAT - 1)) != TRUE)	/* "Message to write: " */
		return (status);
...
int getfence( int f, int n )	/*	the cursor is moved to a matching fence */
{
...
int fmatch(char ch)	/*	Close fences are matched against their partners, and if on screen the cursor briefly lights there		*/
{
	register LINE *oldlp;	/* original line pointer */
	register int oldoff;	/* and offset */
...
int istring( int f, int n )	/* ask for and insert a string into the current buffer at the current point */
{
	register int status;	/* status return code */
	char tstring[NPAT + 1];	/* string to add */

	/* ask for string to insert */
	status = nextarg(TEXT68, tstring, NPAT, sterm);	/* "String to insert: " */
	if (status != TRUE)
		return (status);
...
int ovstring( int f, int n ) 	/* ask for and overwite a string into the current buffer at the current point */
{
	register int status;	/* status return code */
	char tstring[NPAT + 1];	/* string to add */

	/* ask for string to insert */
	status = nextarg(TEXT69, tstring, NPAT, sterm);	/* "String to overwrite: " */
	if (status != TRUE)
		return (status);
