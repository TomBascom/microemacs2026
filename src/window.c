...
void check_resize(int sig)
{
    int row,col;
    struct winsize winsz;

    if (curwp == NULL)		/* not initialized yet */
	return;
    ...
    if ((col != term.t_mcol) || (row != term.t_mrow))
      {
        term.t_mcol = col;
        term.t_mrow = row;
        if (term.t_nrow != term.t_mrow-1)
        { /* Center dot. */
          curwp->w_force = 0;
          curwp->w_flag |= WFFORCE;
...
int nextwind( int f, int n )
{
	...
	if (f) {
		...
		if (n > 0 && n <= nwindows) {
			...
			mlwrite(TEXT203);	/* "Window number out of range" */
			return(FALSE);
		}
	} else
...
int delwind( int f, int n )
{
	...
	if (wheadp->w_wndp == NULL) {
		mlwrite(TEXT204);	/* "Can not delete this window" */
		return(FALSE);
	}
...
int splitwind( int f, int n )
{
	...
	if (curwp->w_ntrows < (modeflag ? 3 : 2)) {
		mlwrite(TEXT205, curwp->w_ntrows);	/* "Cannot split a %d line window" */
		return(FALSE);
	}
...
int enlargewind( int f, int n )
{
	...
	if (wheadp->w_wndp == NULL) {
		mlwrite(TEXT206);		/* "Only one window" */
		return(FALSE);
	}
	...
	if ((adjwp->w_ntrows + (modeflag ? 0 : 1)) <= n) {
		mlwrite(TEXT207);		/* "Impossible change" */
		return(FALSE);
	}
...
int resize( int f, int n )		/* Resize the current window to the requested size */
{
...
int wpopup( BUFFER *popbuf )		/* pop up the indicated buffer */
{
	register WINDOW *wp;
	register BUFFER *bp;
...
int nextup( int f, int n )	/* scroll the next window up (back) a page */
{
	nextwind(FALSE, 1);
	backpage(f, n);
	prevwind(FALSE, 1);
...
int nextdown( int f, int n )	/* scroll the next window down (forward) a page */
{
	nextwind(FALSE, 1);
	forwpage(f, n);
	prevwind(FALSE, 1);
...
int restwnd( int f, int n )	/* restore the saved screen */
{
	...
	mlwrite(TEXT208);			/* "[No such window exists]" */
	return(FALSE);
}

int newsize( int f, int n )	/* resize the screen, re-writing the screen */
{
	...
	if (n < 3 || n > term.t_mrow + 1) {
		mlwrite(TEXT209);		/* "%%Screen size out of range" */
		return(FALSE);
	}
...
int newwidth( int f, int n)	/* resize the screen, re-writing the screen */
{
	register WINDOW *wp;
	...
	if (n < 10 || n > term.t_mcol) {
		mlwrite(TEXT210);			/* "%%Screen width out of range" */
		return(FALSE);
	}
...
int new_col_org( int f, int n)	/* reposition the screen, re-writing the screen */
{
	...
	if (n < 0 || n > term.t_mcol - term.t_ncol) {
		mlwrite(TEXT223);			/* "%%Column origin out of range" */
		return(FALSE);
	}
...
int new_row_org( int f, int n )	/* reposition the screen, re-writing the screen */
{
	/* if the command defaults, assume zero */
	if (f == FALSE)
		n = 0;
	...
	if (n < 0 || n > term.t_mrow - term.t_nrow) {
		mlwrite(TEXT224);			/* "%%Row origin out of range" */
		return(FALSE);
	}
